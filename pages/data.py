...
The database URLs are populated by parsing the wikitext on Wikimedia wikis. URLs generated by a template are usually not included in the database. However, if the URL generating template lies within a citation template under the URL parameter, the bot will be able to resolve the template into a URL, and absorb it.

The bot only retrieves archive URLs from the Wayback Machine, or from pre-existing archive URLs found on the wiki. Users are also able to alter what archive URL to use for any respective URL by `https://iabot.wmcloud.org/index.php?page=manageurlsingle`.

The replication DB is maintained/updated in near real-time to the bot's production DB. The bot maintains its database of pages and URLs as it cycles through a wiki's articles from start to finish. On particularly large wikis, this means it can take months for a page to be visited and subsequent data to be updated. This can be expedited by simply running the bot on-demand by visiting `https://iabot.wmcloud.org/index.php?page=runbotsingle`.

URL states update no sooner than 3 days from the last time of scan. They are only updated if the URL is encountered in a page the bot is actively assessing.

The DB only maintains a wiki's page by its numerical ID. To lookup the page ID, you can click on the "Page Information" link on the article. If the classic "Vector" skin is being used, the older theme deprecated in 2022/2023, the link can be found on the left side panel under "Tools". If using the latest skin/theme, the "Tools" is a dropdown found in the top right of the article.
"""
